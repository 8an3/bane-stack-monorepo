...
import { Dialog, DialogContent, DialogClose, Popover, PopoverTrigger, PopoverContent, Tabs, TabsList, TabsTrigger, TabsContent, } from '~/components/ui';
...
export   function StoreNavSection() {
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="sm:max-w-xs h-full p-0">
          ...
          <Tabs defaultValue="women" className="mt-2">
            ...
            {navigation.categories.map((category) => (
              <TabsContent key={category.name} value={category.id} className="space-y-10 px-4 pb-8 pt-10">
                  ...
                  {navigation.categories.map((category) => (
                    <Popover key={category.name}>
                      <PopoverTrigger className="group relative flex items-center justify-center text-sm font-medium text-gray-700 transition-colors duration-200 ease-out hover:text-gray-800 data-[state=open]:text-indigo-600">
                        ...
                        <span
                          aria-hidden="true"
                          className="absolute inset-x-0 -bottom-px z-30 h-0.5 transition duration-200 ease-out group-data-[state=open]:bg-indigo-600"
                        />
                      </PopoverTrigger>
                      <PopoverContent
