...
export function DisplayInstallCodeArray({ title = 'Prerequisites', libModule = [], desc = null }) {
    ...
    const copyToClipboard = (library) => {
        let fullCmd = '';
        const moduleString = Array.isArray(libModule) ? libModule.join(' ') : libModule;

        switch (library) {
            case 'npm':
                fullCmd = `npm install ${globalValue ? '-g ' : ''}${moduleString}`.replace(/\s+/g, ' ').trim();
                break;
            case 'pnpm':
                fullCmd = `pnpm install ${globalValue ? '-g ' : ''}${moduleString}`.replace(/\s+/g, ' ').trim();
                break;
            case 'yarn':
                fullCmd = `yarn ${globalValue ? 'global ' : ''}add ${moduleString}`.replace(/\s+/g, ' ').trim();
                break;
            case 'bun':
                fullCmd = `bun install ${globalValue ? '-g ' : ''}${moduleString}`.replace(/\s+/g, ' ').trim();
                break;
            default:
                fullCmd = `npm install ${globalValue ? '-g ' : ''}${moduleString}`.replace(/\s+/g, ' ').trim();
        }
    ...
    useEffect(() => {
        ...
        switch (lib) {
            case 'npm':
                cmdText = `npm install ${globalValue ? '-g ' : ''}${moduleString}`;
                break;
            case 'pnpm':
                cmdText = `pnpm install ${globalValue ? '-g ' : ''}${moduleString}`;
                break;
            case 'yarn':
                cmdText = `yarn ${globalValue ? 'global ' : ''}add ${moduleString}`;
                break;
            case 'bun':
                cmdText = `bun install ${globalValue ? '-g ' : ''}${moduleString}`;
                break;
            default:
                cmdText = `npm install ${globalValue ? '-g ' : ''}${moduleString}`;
        }
